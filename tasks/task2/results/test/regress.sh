...
echo "🧪 Загрузка фикстур нового сервиса..."
PGPASSWORD="${DB_PASSWORD}" psql -h "${NEW_DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures-new.sql
...
curl -sSf "${BASE}/api/bookings?userId=test-user-2" | grep -q 'test-user-2' && pass "Все бронирования получены" || fail "Бронирования не получены"
